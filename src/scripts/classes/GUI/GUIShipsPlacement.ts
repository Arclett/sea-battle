import { shipsImages, imagesPlacementMenu, GameMode, PlacementStatus } from "../../types/enums";
...
export class GUIShipsPlacement {
    ...
    ShipCoordinatesWithBackground = new ShipsCoordinatesWithBackground();
    private ShipControl = new ShipsControl();
    mode: GameMode;
    enemyStatus: PlacementStatus;
    enemyPlacement: ShipsData;
    ...
    renderShipsPlacement(fieldSkin: string) {
        ...
        const startButton = Element.createElement({
            tag: "button",
            id: "startGame",
    ...
    checkCoords() {
        if (
            this.ShipCoordinatesWithBackground.getShipsCoordinatesWithBackground().ShipsCoordinates.coordinatesOfShips
                .length === 10
        ) {
            return true;
        }
